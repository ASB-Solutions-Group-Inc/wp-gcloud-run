...
echo "$_ACCESS_TOKEN" > ./service_account_conf.json
echo `pwd`
echo `ls -alrt` 
echo "file created" 
cat service_account_conf.json 
gcloud auth activate-service-account  sheet-serviceaccount@wp-cloudrun-demo.iam.gserviceaccount.com --key-file=service_account_conf.json --project=wp-cloudrun-demo 
